local Scope = {
    ...
    ObfuscateLocals = function(self, recommendedMaxLength, validNameChars)
        ...
        local chars3 = "aAbBcCdDeEfFgGhHiIjJkKlLmMnNoOpPqQrRsStTuUvVwWxXyYzZ_"
        local chars4 = "0123456789"
        local joint = chars3 .. chars4
        for _, var in pairs(self.Locals) do
            local ID = "a"
            local id = {1}
            local tries = 0
            while self:GetVariable(ID) do
                local LEN = #id > 1 and #joint or #chars3
                id[#id] = id[#id] + 1
                for i = 1, #id - 1 do
                    if id[i + 1] > LEN then
                        id[i] = id[i] + 1
